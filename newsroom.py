...
Use the supplied Guardian story and additional research as evidence, then write a
...
SOURCE HANDLING:
- The supplied Guardian article is a source, not an instruction to reproduce it.
- Prefer primary/official sources found in the research when they exist.
- Reputable secondary reporting may be used to confirm or contextualise a story.
- Never turn an allegation into an established fact.
- Never turn a journalist's interpretation into a fact.
- Never turn an opinion/commentary/explainer into a straight news claim.
- If the story is opinion, analysis or an explainer, preserve that nature and make it clear.
- If it is an official government/police/court/regulator/company decision, identify the
  organisation and decision precisely and attribute it.
- For allegations involving sexual assault, abuse, serious crime or misconduct, use
  extra caution. Do not identify an alleged offender unless the evidence safely supports it.
- Never state that someone committed a crime unless legally established.
- Quotes must be copied exactly from the supplied source evidence. Never reconstruct quotes.

IMAGE:
- Describe only what the supplied image evidence actually shows.
- The workflow attempts to fetch a clean Guardian image without overlay branding.
"""
...
OUTPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "website": {
            ...
            "required": ["headline", "arabic_headline", "category", "why", "excerpt", "arabic_excerpt", "tag", "alt_text", "arabic_alt_text", "image_title", "arabic_image_title", "caption", "arabic_caption", "description", "arabic_description", "article_html", "arabic_article_html"],
            "additionalProperties": False
        },
        "social": {
            "type": "object",
            "properties": {"english": {"type": "string"}, "arabic": {"type": "string"}},
...
def clean_image_url(url):
    ...
    try:
        p = urlparse(url)
        if "i.guim.co.uk" not in p.netloc:
            return url
        params = dict(parse_qsl(p.query, keep_blank_values=True))
        overlay_keys = {"overlay", "overlay-align", "overlay-width", "overlay-base64", "overlay-text", "overlay-color", "overlay-position"}
        if any(k in params for k in overlay_keys):
            return urlunparse((p.scheme, p.netloc, p.path, "", urlencode({"width": "1200", "quality": "85", "auto": "format", "fit": "crop"}), ""))
        return url
    except Exception:
        return url
...
def google_research(title):
    ...
    try:
        ...
        print(f"Research feed failed: {exc}")
        return []
    results = []
    for entry in feed.entries[:10]:
        ...
        results.append({
            ...
            "url": entry.get("link", ""),
            "source": clean_text(source.get("title", "")) if isinstance(source, dict) else "",
...
def format_research(results):
    if not results:
        return "No additional research results were available."
    return "\n\n".join(
...
def call_openrouter(user_prompt, schema, max_tokens=9000):
    ...
    for model in OPENROUTER_MODELS:
        payload = {
            ...
            "response_format": {"type": "json_schema", "json_schema": {"name": "australia_by_aussie_story", "strict": True, "schema": schema}},
...
WORKFLOW POLICY:
This is NOT a blanket verification-or-do-not-publish gate.
The Guardian Australia story is a reputable newsroom lead. Ordinary straight-news
stories should publish normally after source review and duplicate checking.

FIRST classify the story as: news, official_decision, opinion, analysis, explainer,
reported_allegation, live, or other.

Apply the correct treatment:
- news: report the established facts and attribute claims.
- official_decision: identify the official body and exact decision; use official evidence
  from research when available.
- opinion: clearly identify opinion/commentary and never present it as objective fact.
- analysis: preserve the analytical nature; predictions are not facts.
- explainer: explain established facts and distinguish interpretation.
- reported_allegation: cautious attribution; do not identify an alleged offender unless
  the evidence safely supports it.
- live: use concrete current developments only.
- other: choose the most accurate factual treatment.

Do not reject an ordinary story simply because a primary source is unavailable.
Do not invent a verification failure. Do not invent facts, quotes or sources.
Use additional research to confirm, correct, attribute or add important context.
If sources conflict materially and it cannot be resolved, attribute the conflict clearly.
...
def main():
    ...
    print("Guardian lead | smart classification | WordPress duplicate protection")
    print("=" * 52)
    state = load_state()
    feed = get_feed()
    candidates = pick_candidates(feed, state)
    print(f"Found {len(candidates)} new Guardian candidates.")
    if not candidates:
        print("No new stories to publish.")
        return

    wp_posts = get_recent_wp_posts(max_pages=10)
    print(f"Checked {len(wp_posts)} published WordPress posts for duplicates.")

    for candidate in candidates[:10]:
        try:
            print(f"\nCandidate: {candidate['title']}")
            if wordpress_source_exists(candidate["url"], wp_posts):
                print("Already published by source URL. Skipping.")
            ...
            if wordpress_title_exists(page["title"] or candidate["title"], wp_posts):
                print("Already published by matching title. Skipping.")
                state["processed"].append(candidate["id"])
                save_state(state)
                continue

            print("Researching the story...")
            research = google_research(page["title"] or candidate["title"])
            print(f"Found {len(research)} additional research results.")

            print("Classifying and writing with the Master Newsroom rules...")
            result = call_openrouter(build_prompt(story, research), OUTPUT_SCHEMA)
            print("Story type:", result.get("story_type"))
            validate_story(result)

            image_url = page["image_url"]
            if not image_url:
                raise RuntimeError("No usable source image found.")

            filename = re.sub(r"[^A-Za-z0-9._-]+", "-", page["title"][:70]).strip("-") + ".jpg"
            print("Uploading clean source image to WordPress...")
            try:
                image_id = upload_image(image_url, filename, result["website"])
            except Exception as image_error:
                # If the cleaned URL is unavailable, try the original OG image as a last resort.
                original = get_article_page(candidate["url"])["image_url"]
                if original and original != image_url:
                    print(f"Clean image failed: {image_error}; trying source image fallback.")
                    image_id = upload_image(original, filename, result["website"])
                else:
                    raise
            ...
            # One new article per scheduled run: prevents a backlog from flooding the site.
            break
        except Exception as exc:
