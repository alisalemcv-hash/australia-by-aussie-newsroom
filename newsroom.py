...
def official_research(title, story_text):
    """Search only for an official/primary source. Never used for ordinary news."""
    ...
    for entry in feed.entries[:8]:
        ...
        results.append({
            ...
            "summary": clean_text(entry.get("summary", ""))[:1800],
        })
        if len(results) >= 3:
            break
    return results
...
def wordpress_source_exists(source_url, posts):
    return any(source_marker(source_url) in p.get("content", {}).get("rendered", "") for p in posts)
...
def upload_image(image_url, filename, metadata):
    ...
    meta = requests.post(
        f"{endpoint}/{media_id}",
        auth=wp_auth(),
        json={"alt_text": metadata["alt_text"], "title": metadata["image_title"], "caption": metadata["caption"], "description": metadata["description"]},
        timeout=30,
    )
...
def validate_story(story):
    ...
    if len(BeautifulSoup(website["article_html"], "html.parser").get_text(" ", strip=True)) < 300:
...
def main():
    ...
    print("Guardian lead | original writing | targeted official-source check")
    print("=" * 52)
    ...
    for candidate in candidates[:10]:
        try:
            ...
            if wordpress_source_exists(candidate["url"], wp_posts):
                print("Already published by source URL. Skipping before AI/research.")
            ...
            if wordpress_title_exists(page["title"] or candidate["title"], wp_posts):
                print("Already published by matching title. Skipping before AI/research.")
                state["processed"].append(candidate["id"])
                save_state(state)
                continue

            # FIRST AI PASS: classification + original article. No general research.
            print("Writing and classifying from Guardian source only...")
            result = call_openrouter(build_prompt(story), OUTPUT_SCHEMA)
            story_type = result.get("story_type", "news")
            print("Story type:", story_type)

            # Only official decisions trigger targeted official-source research.
            if story_type == "official_decision":
                print("Official decision detected. Searching only for an official/primary source...")
                official_sources = official_research(page["title"] or candidate["title"], page.get("text", ""))
                print(f"Found {len(official_sources)} official/primary sources.")
                result = call_openrouter(build_prompt(story, official_sources), OUTPUT_SCHEMA)
                print("Story type after official-source review:", result.get("story_type"))
